...
"""
PyInstaller 打包配置文件 - CEF Python 桌面应用（局域网版本）
支持 Win7 兼容
使用方法: pyinstaller build_cef_lan.spec
"""
...
a = Analysis(
    ['backend/app_cef_lan.py'],  # 使用局域网版本
    pathex=[],
    binaries=[],
    datas=[
        # 数据库目录
...
def copy_cef_files():
    """复制 CEF 资源文件到打包目录"""
    dist_dir = os.path.join(os.getcwd(), 'dist', '文件管理系统_LAN')
...
def copy_cryptography_dlls():
    ...
    for dll_name in openssl_dlls:
        src = os.path.join(dist_dir, dll_name)

        if os.path.exists(src):
            # 复制到主目录（通常已由 PyInstaller 完成）
            # 同时复制到 cryptography 子目录（关键！）
    ...
    if copied_count > 0:
        print(f"✓ 已复制 {copied_count} 个 cryptography DLL 文件到正确位置")
    else:
        print("⚠ 未找到 OpenSSL DLL 文件，检查打包目录")
